exports.handler = async function (event) {
  ...
  const hasProfile = (goals && goals.trim()) || (avoids && avoids.trim()) || (conditions && conditions.trim());
  const conditionGuidance = conditions ? buildConditionGuidance(conditions) : '';

  const genericGuidance = ` No profile given — use general nutrition literacy: flag well-known controversial ingredients (artificial dyes, trans fats, high fructose corn syrup, palm oil), caution items worth general awareness (added sugars, sodium, seed oils, artificial sweeteners, gums), positive for genuinely beneficial whole-food ingredients (fiber, protein, vegetables, whole grains), neutral for everything else unremarkable.`;
...
flag=conflicts with avoids/goals/conditions. positive=helps goals. caution=notable. neutral=fine. section=flagged only if flag, else other. Keep parent ingredients whole. Ignore "contains X% or less of".${hasProfile ? conditionGuidance : genericGuidance}`;

  const conditionsPart = conditions ? `\nHealth conditions: ${conditions}.` : '';
  const profilePart = hasProfile
    ? `\nGoals: ${goals || "none"}.\nAvoiding: ${avoids || "nothing"}.${conditionsPart}`
    : `\nNo personal profile provided — use general nutrition literacy.`;

  let messageContent;
  if (imageBase64 && imageType) {
    messageContent = [
      { type: "image", source: { type: "base64", media_type: imageType, data: imageBase64 } },
      { type: "text", text: `This is a photo of a food product label. Extract the ingredient list from the image and analyze each ingredient.${profilePart}` }
    ];
  } else {
